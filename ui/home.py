...
# A ordem/ícones/nomes do stepper de progresso vivem em
# components.BarraProgresso.ETAPAS (ui/components.py) — única fonte de
# verdade para as etapas conhecidas do pipeline.
ROTULOS_CONSOLE = {
...
def render_pagina():
    ...
    components.render_section_eyebrow("⚙️", "Configuração da Auditoria")
    ...
    components.render_section_eyebrow("📡", "Execução do Pipeline")
    with st.status("🐕 Cerberus está farejando os módulos...", expanded=True) as status:
        ...
        # Callback real que o pipeline invoca a cada etapa concluída.
        # Contrato preservado: recebe (etapa: str, detalhe: str).
    ...
    with st.container(border=True):
        ...
        # st.markdown puro (sem wrapper HTML) para preservar a renderização
        # nativa de tabelas e blocos de código do parecer da IA.
